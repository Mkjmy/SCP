...
def main_loop(stdscr):
    ...
    for _ in range(200):
        ...
        dept = "Security" if role in ["Guard", "ISD Agent"] else "Research & Science" if role == "Scientist" else "D-Class" if role == "D-Class" else "Engineering"
    ...
    while not game_over:
        ...
        # --- TURN PROGRESSION ---
        if verb not in ["inventory", "look", "quit"]:
            current_story_msgs = [] 
            turn_counter += 1
            sim_log = npc_manager.process_npc_sim_turn(npc_manager._npcs, player, game_map)

            # Story Check
            triggered = story_manager.check_events(turn_counter, player, npc_manager, scp_manager, game_map)
            if triggered:
                ...
                if "spawns" in triggered:
                    for spawn_info in triggered["spawns"]:
                        ...
                        if nid:
                            ...
                            if "clearance" in spawn_info: char.clearance_level = spawn_info["clearance"]
            ...
            # CRITICAL SYNC: Re-fetch state after potential movement
            current_room = game_map[player.location]
            npcs_in_room = npc_manager.get_npcs_in_room(player.location)
            scps_in_room = scp_manager.get_scps_in_room(player.location)
        ...
        if action == 'quit': game_over = True
            ...
            message_to_show = "You sit down and consume a tray of lukewarm, sterile Foundation rations."
            player.health = min(player.max_health, player.health + 5)
        elif action == 'rest':
            message_to_show = "You lie down for a moment, feeling a bit more alert."
            ...
            if player.attributes['intelligence'] + random.randint(1, 6) > difficulty:
                if npc_manager._npcs:
                    ...
                    player.known_secrets[random_nid][stype] = "Classified"
                    message_to_show = f"SUCCESS: You discovered leverage against {npc_char.name}."
            else: message_to_show = "You failed to find anything useful."
        elif action == 'request medical aid':
            message_to_show = "A medical technician patches you up."
            player.health = min(player.max_health, player.health + 30)
        elif verb == 'inventory': message_to_show = player.get_description()
        elif verb == 'look':
            found = False
            ...
            if chosen != "back":
                ...
                if target_char:
                    ...
                    if topic == "manipulate using secrets":
                        ...
                        else: message_to_show = f"{target_char.name} dismisses your threats."
            ...
            message_to_show = msg if not success else f"You move {target}."
        elif verb == 'attack':
        ...
        if not message_to_show and action != 'quit' and not current_story_msgs and verb != "STORY_CHOICE":

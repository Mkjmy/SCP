...
def main_loop(stdscr):
    ...
    while not game_over:
        ...
        if action in display_options and active_story_event:
            ...
            active_story_event = None; current_story_msgs = []; continue
        ...
        if action == 'quit': game_over = True
            ...
            for n in npcs_in_room:
                if target.lower() in n["character"].name.lower():
                    n["character"].is_focused = True
                    n["character"].focus_timer = 5
            ...
            if chosen != "back":
                ...
                target_scp = next((s for s in scps_in_room if name in s.id.lower()), None)
                
                if target_char:
                    topics = ["ask about current duties", "try to socialize", "ask about facility secrets", "gossip about others", "back"]
                    ...
                    if topic != "back":
                        message_to_show = target_char.get_contextual_dialogue(topic)
                elif target_scp:
                    res = target_scp.on_player_talk(player, None)
            ...
            if chosen != "back":
                ...
                target_char = next((n["character"] for n in npcs_in_room if name in n["character"].name.lower()), None)
                target_scp = next((s for s in scps_in_room if name in s.id.lower()), None)
                message_to_show, is_fatal = attack(player, [target_char] if target_char else [], scps_in_room=[target_scp] if target_scp else [])
                current_story_msgs = []
